function [map,map_mu,register,D,ms_map,ms_mu,ms_register] = my_QCIR(moving,static,varargin)
% QCIR Algorithm
%
% Inputs 
...
for k = level:-1:2
    ...
    [temp_map,temp_mu,temp_register] = my_registration_lm(ms_face{k},ms_vertex{k},initial_map,...
        initial_register,ms_static{k},gaussian_size);

    [temp_map,temp_mu] = my_QCLR(ms_face{k},ms_vertex{k},size(ms_static{k}),'mu',temp_mu);
...
[map,map_mu,register] = my_registration_lm(ms_face{1},ms_vertex{1},initial_map,...
        initial_register,ms_static{1},gaussian_size);
    
[map,map_mu] = my_QCLR(ms_face{1},ms_vertex{1},size(ms_static{1}),'mu',map_mu);

targetsize = size(ms_static{1});
%% 获取x-y方向位移形变场
initial_mesh = ms_vertex{1};
deformed_mesh = map;
...
if plotflag
    ...
    % 
    figure
    imshowpair(static,moving,'montage'); title('Static Image,Moving Image;');
    % hold on;
